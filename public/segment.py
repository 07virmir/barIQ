...
'''
def evalPath(ePath='../data/samples'):
    outDict = {}
    for path in os.listdir(ePath):
        print(path)
        if 'green' in path or 'bq' in path or 'groups' in path or path[0] == '.':
            #print('skip: ', path)
            continue
        imagePath = os.path.join(ePath, path)
        tDict = evalImage(imagePath)
        for b in tDict:
            tb = str(b)
            if tb in outDict:
                outDict[tb] += tDict[b]
